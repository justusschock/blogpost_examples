...
class FacialAgeClassificationDataset(torch.utils.data.Dataset):
    def __init__(self, path: str, download: Optional[bool] = None):
        ...
        if download:
            ...
            api.dataset_download_files('frabbisw/facial-age', path=path, quiet=False, unzip=True)
        ...
        for idx, directory in enumerate(sorted([x for x in os.listdir(path) if os.path.isdir(os.path.join(path, x))])):
            ...
            for file in sorted([x for x in os.listdir(os.path.join(path, directory)) if os.path.isfile(os.path.join(path, directory, x)) and x.endswith('.png')]):
...
class LitModule(pl.LightningModule):
    def __init__(self, num_classes: int, pretrained: bool = True):
        ...
        self.classification_head = torch.nn.Linear(self.model.fc.in_features, num_classes)
...
if __name__ == '__main__':
    ...
    trainset_clf, validationset_clf = torch.utils.data.random_split(dataset_classification, [trainset_clf_length, valset_clf_length])
    trainset_reg, validationset_reg = torch.utils.data.random_split(dataset_regression, [trainset_reg_length, valset_reg_length])

...
class MelanomaImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        image = read_image(os.path.join(self.root_dir, self.annotations.iloc[index, 0]+".jpg"))
...
class MelanomaCSVDataset(torch.utils.data.Dataset):
    def __init__(self, csv_file: str) -> None:
        self.annotations = pd.read_csv(csv_file).dropna()
        self.sex_class_embedding = {'male': 0., 'female': 1.}
...
class MyLitModel(pl.LightningModule):
    def __init__(self) -> None:
        ...
        self.mlp_head = torch.nn.Sequential(torch.nn.Linear(4, 8), torch.nn.ReLU(), torch.nn.Linear(8, 8), torch.nn.ReLU(), torch.nn.Linear(8, 2))
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        y_hat = self.mlp_head(torch.cat([y_img, x_sex.unsqueeze(1), x_age.unsqueeze(1)], dim=-1))
...
if __name__ == '__main__':
    ...
    if not os.path.exists(dataset_path) or not os.path.exists(os.path.join(dataset_path, 'siim-isic-melanoma-classification')):
        ...
        if not os.path.isfile(zip_path):
            api.competition_download_files('siim-isic-melanoma-classification',
                                    path=dataset_path,
                                    quiet=False)
    ...
    dataset = CombinedMelanomaDataset(csv_file=os.path.join(dataset_path, 'train.csv'), root_dir=os.path.join(dataset_path, 'jpeg', 'train'))
    ...
    trainset, valset = torch.utils.data.random_split(dataset, [int(len(dataset) * 0.75), len(dataset) - int(len(dataset)*0.75)])

    trainloader = torch.utils.data.DataLoader(trainset, batch_size=64, num_workers=8, pin_memory=True, shuffle=True)
    valloader = torch.utils.data.DataLoader(valset, batch_size=64, num_workers=8, pin_memory=True, shuffle=False)
